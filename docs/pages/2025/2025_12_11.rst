...
- Developed a fitting object that reads an hkl file, bins spots based on
  resolution and then performs the fitting (as in the original paper
  :cite:`clabbers_2019`). The fitter then performs a scaling correction on the
  original data. The only problem is that the corrected datasets have lower
  :math:`R_1` when processed with :code:`shelxt`.
...
- Compute :math:`R_1` per image to identify images that are highly impacted by
  dynamical effects.
- Retrain the gradient boosted regressor (GBR) to include more information about
...
- Read about `PointNet Architecture <point_nets_>`_.
